...
def formatted_print(snapshot, phase, params, duration, algo_name):
  print('======================================================')
...
def train_single_epoch(feats, mos, param_grid, rnd_seed, algo_name):
    t_start = time.time()
    
    # grid search for parameter
    grid = RandomizedSearchCV(SVR(), param_grid, cv=10, n_jobs=-1, random_state=rnd_seed)#cv=3
    ...
    t_end = time.time()
    formatted_print(metrics, phase='train', params=best_params, duration=(t_end - t_start), algo_name = algo_name)
...
if __name__=='__main__':
    ...
    feature_path = r'./feats/selected_340'
    label_path = r'./data/label'
    out_path = r'./pred/namefea_baseline_feas340_cv10_grid'
    algo_name = 'DFGC1st_withstd_feats340'
    ...
    param_grid = {"kernel": ("linear", 'rbf'), "C": np.logspace(-4, 4, 9), "gamma": np.logspace(-4, 4, 9)}
    rnd_seed = 42
    model = train_single_epoch(feats_train, mos_train, param_grid, rnd_seed, algo_name)
